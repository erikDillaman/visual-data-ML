...
const FEATURE_SUGGESTIONS = [
  { emoji: '🏅', name: 'Reputation' },
  { emoji: '⚡', name: 'Special Ability' },
  { emoji: '🌙', name: 'Active Time' },
  { emoji: '🤝', name: 'Attitude to Player' },
  { emoji: '🏰', name: 'Social Class' },
];

const CATEGORY_PRESET_MAP = {
  reputation: [
    ['Respected', 'Feared', 'Unknown', 'Outcast'],
    ['Heroic', 'Villainous', 'Neutral', 'Mysterious'],
  ],
  ability: [
    ['Magic', 'Stealth', 'Brute Force', 'Healing', 'None'],
    ['Elemental', 'Physical', 'Mental', 'Support', 'None'],
  ],
  time: [
    ['Day', 'Night', 'Both'],
    ['Dawn', 'Dusk', 'Midday', 'Midnight', 'Any Time'],
  ],
  attitude: [
    ['Helpful', 'Hostile', 'Indifferent', 'Unpredictable'],
    ['Allied', 'Enemy', 'Neutral', 'Conditional'],
  ],
  class: [
    ['Noble', 'Commoner', 'Outcast', 'Mystical'],
    ['Royalty', 'Merchant', 'Soldier', 'Peasant', 'Wanderer'],
  ],
};

function getPresetsForFeature(name) {
  const n = name.toLowerCase();
  if (n.includes('reput') || n.includes('fame') || n.includes('known')) return CATEGORY_PRESET_MAP.reputation;
  if (n.includes('abilit') || n.includes('power') || n.includes('skill') || n.includes('special')) return CATEGORY_PRESET_MAP.ability;
  if (n.includes('time') || n.includes('active') || n.includes('when') || n.includes('hour')) return CATEGORY_PRESET_MAP.time;
  if (n.includes('attitude') || n.includes('player') || n.includes('relation') || n.includes('toward')) return CATEGORY_PRESET_MAP.attitude;
  if (n.includes('class') || n.includes('social') || n.includes('rank') || n.includes('status')) return CATEGORY_PRESET_MAP.class;
  return [
...
function inferCategory(item, featureName, categories) {
  ...
  if (n.includes('reput') || n.includes('fame') || n.includes('known')) {
    if (item.alignment === 'Friend') return best('Respected', 'Heroic', 'Allied') || categories[0];
    if (item.alignment === 'Foe')   return best('Feared', 'Villainous', 'Enemy') || categories[1 % categories.length];
    return best('Unknown', 'Neutral', 'Mysterious') || categories[categories.length - 1];
  }
  if (n.includes('abilit') || n.includes('power') || n.includes('skill') || n.includes('special')) {
    if (item.type === 'Elemental') return best('Magic', 'Elemental', 'Mental') || categories[0];
    if (item.type === 'Undead')    return best('Stealth', 'Physical', 'None') || categories[1 % categories.length];
    if (item.type === 'Spirit')    return best('Magic', 'Elemental', 'Support') || categories[0];
    if (item.strength === 'Strong' || item.strength === 'Powerful') return best('Brute Force', 'Physical') || categories[2 % categories.length];
    if (item.speaks)               return best('Healing', 'Support', 'Mental') || categories[Math.floor(categories.length / 2)];
    return best('None', 'Physical') || categories[categories.length - 1];
  }
  if (n.includes('time') || n.includes('active') || n.includes('when') || n.includes('hour')) {
    if (item.type === 'Undead')    return best('Night', 'Midnight', 'Dusk') || categories[1 % categories.length];
    if (item.type === 'Spirit')    return best('Night', 'Dusk', 'Dawn') || categories[1 % categories.length];
    if (item.territory === 'Dungeon') return best('Night', 'Midnight', 'Both') || categories[1 % categories.length];
    return best('Day', 'Both', 'Any Time', 'Midday') || categories[0];
  }
  if (n.includes('attitude') || n.includes('player') || n.includes('relation') || n.includes('toward')) {
    if (item.alignment === 'Friend')  return best('Helpful', 'Allied', 'Conditional') || categories[0];
    if (item.alignment === 'Foe')     return best('Hostile', 'Enemy') || categories[1 % categories.length];
    return best('Indifferent', 'Neutral', 'Unpredictable', 'Conditional') || categories[2 % categories.length];
  }
  if (n.includes('class') || n.includes('social') || n.includes('rank') || n.includes('status')) {
    if (item.type === 'Human' && item.territory === 'Village') return best('Commoner', 'Peasant', 'Merchant') || categories[1 % categories.length];
    if (item.disposition === 'Calm' && item.speaks)            return best('Noble', 'Royalty', 'Merchant') || categories[0];
    if (item.type !== 'Human')                                 return best('Mystical', 'Wanderer', 'Outcast') || categories[categories.length - 1];
    return best('Outcast', 'Wanderer', 'Commoner') || categories[2 % categories.length];
  }
...
const ITEMS = [
  // — Friends —
  { id: 201, name: "Village Elder",    type: "Human",     disposition: "Calm",       strength: 2,  territory: "Village", speaks: true,  alignment: "Friend"  },
  { id: 202, name: "Guard Captain",    type: "Human",     disposition: "Wary",       strength: 7,  territory: "Village", speaks: true,  alignment: "Friend"  },
  { id: 203, name: "Traveling Healer", type: "Human",     disposition: "Calm",       strength: 1,  territory: "Village", speaks: true,  alignment: "Friend"  },
  { id: 204, name: "Forest Ranger",    type: "Human",     disposition: "Wary",       strength: 5,  territory: "Forest",  speaks: true,  alignment: "Friend"  },
  { id: 205, name: "Young Knight",     type: "Human",     disposition: "Curious",    strength: 8,  territory: "Village", speaks: true,  alignment: "Friend"  },
  { id: 206, name: "River Spirit",     type: "Spirit",    disposition: "Calm",       strength: 4,  territory: "Forest",  speaks: true,  alignment: "Friend"  },
  { id: 207, name: "Fairy Guide",      type: "Spirit",    disposition: "Curious",    strength: 2,  territory: "Forest",  speaks: true,  alignment: "Friend"  },
  { id: 208, name: "Wise Owl",         type: "Beast",     disposition: "Calm",       strength: 3,  territory: "Forest",  speaks: false, alignment: "Friend"  },
  { id: 209, name: "Dwarf Smith",      type: "Human",     disposition: "Calm",       strength: 7,  territory: "Village", speaks: true,  alignment: "Friend"  },
  { id: 210, name: "Ghost Monk",       type: "Undead",    disposition: "Calm",       strength: 5,  territory: "Ruins",   speaks: true,  alignment: "Friend"  },
  { id: 211, name: "Snow Fox",         type: "Beast",     disposition: "Curious",    strength: 2,  territory: "Forest",  speaks: false, alignment: "Friend"  },
  { id: 212, name: "Light Elemental",  type: "Elemental", disposition: "Calm",       strength: 8,  territory: "Village", speaks: false, alignment: "Friend"  },
  { id: 213, name: "Village Merchant", type: "Human",     disposition: "Curious",    strength: 1,  territory: "Village", speaks: true,  alignment: "Friend"  },
  { id: 214, name: "Baby Dragon",      type: "Beast",     disposition: "Curious",    strength: 6,  territory: "Volcano", speaks: false, alignment: "Friend"  },
  // — Foes —
  { id: 215, name: "Dark Wraith",      type: "Undead",    disposition: "Aggressive", strength: 7,  territory: "Dungeon", speaks: false, alignment: "Foe"     },
  { id: 216, name: "Goblin Raider",    type: "Beast",     disposition: "Aggressive", strength: 3,  territory: "Dungeon", speaks: false, alignment: "Foe"     },
  { id: 217, name: "Orc Warlord",      type: "Human",     disposition: "Aggressive", strength: 9,  territory: "Dungeon", speaks: true,  alignment: "Foe"     },
  { id: 218, name: "Shadow Witch",     type: "Human",     disposition: "Menacing",   strength: 8,  territory: "Ruins",   speaks: true,  alignment: "Foe"     },
  { id: 219, name: "Lava Serpent",     type: "Beast",     disposition: "Aggressive", strength: 7,  territory: "Volcano", speaks: false, alignment: "Foe"     },
  { id: 220, name: "Bone Archer",      type: "Undead",    disposition: "Aggressive", strength: 5,  territory: "Ruins",   speaks: false, alignment: "Foe"     },
  { id: 221, name: "Cursed Knight",    type: "Undead",    disposition: "Menacing",   strength: 8,  territory: "Dungeon", speaks: true,  alignment: "Foe"     },
  { id: 222, name: "Fire Demon",       type: "Elemental", disposition: "Aggressive", strength: 10, territory: "Volcano", speaks: false, alignment: "Foe"     },
  { id: 223, name: "Plague Rat",       type: "Beast",     disposition: "Aggressive", strength: 2,  territory: "Dungeon", speaks: false, alignment: "Foe"     },
  { id: 224, name: "Stone Golem",      type: "Elemental", disposition: "Aggressive", strength: 9,  territory: "Ruins",   speaks: false, alignment: "Foe"     },
  { id: 225, name: "Bandit Chief",     type: "Human",     disposition: "Menacing",   strength: 7,  territory: "Forest",  speaks: true,  alignment: "Foe"     },
  { id: 226, name: "Vengeful Spirit",  type: "Spirit",    disposition: "Menacing",   strength: 4,  territory: "Ruins",   speaks: false, alignment: "Foe"     },
  { id: 227, name: "Vampire Lord",     type: "Undead",    disposition: "Menacing",   strength: 10, territory: "Dungeon", speaks: true,  alignment: "Foe"     },
  // — Neutral —
  { id: 228, name: "Wild Boar",        type: "Beast",     disposition: "Wary",       strength: 5,  territory: "Forest",  speaks: false, alignment: "Neutral" },
  { id: 229, name: "Cave Troll",       type: "Beast",     disposition: "Wary",       strength: 7,  territory: "Dungeon", speaks: false, alignment: "Neutral" },
  { id: 230, name: "Desert Nomad",     type: "Human",     disposition: "Wary",       strength: 4,  territory: "Ruins",   speaks: true,  alignment: "Neutral" },
  { id: 231, name: "Wandering Mage",   type: "Human",     disposition: "Curious",    strength: 6,  territory: "Forest",  speaks: true,  alignment: "Neutral" },
  { id: 232, name: "Mountain Goat",    type: "Beast",     disposition: "Wary",       strength: 2,  territory: "Forest",  speaks: false, alignment: "Neutral" },
  { id: 233, name: "Fog Elemental",    type: "Elemental", disposition: "Wary",       strength: 5,  territory: "Forest",  speaks: false, alignment: "Neutral" },
  { id: 234, name: "Ancient Statue",   type: "Elemental", disposition: "Calm",       strength: 8,  territory: "Ruins",   speaks: false, alignment: "Neutral" },
  { id: 235, name: "Lost Child",       type: "Human",     disposition: "Curious",    strength: 1,  territory: "Dungeon", speaks: true,  alignment: "Neutral" },
  { id: 236, name: "Sea Turtle",       type: "Beast",     disposition: "Calm",       strength: 4,  territory: "Forest",  speaks: false, alignment: "Neutral" },
  { id: 237, name: "Storm Hawk",       type: "Beast",     disposition: "Wary",       strength: 5,  territory: "Forest",  speaks: false, alignment: "Neutral" },
  { id: 238, name: "Ruin Guard",       type: "Undead",    disposition: "Wary",       strength: 7,  territory: "Ruins",   speaks: false, alignment: "Neutral" },
  { id: 239, name: "Sand Spirit",      type: "Spirit",    disposition: "Wary",       strength: 4,  territory: "Ruins",   speaks: false, alignment: "Neutral" },
  { id: 240, name: "Traveling Bard",   type: "Human",     disposition: "Curious",    strength: 2,  territory: "Village", speaks: true,  alignment: "Neutral" },
];
