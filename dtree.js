...
function _renderTestTree(leafCounts) {
  ...
  function drawEdges(node) {
    ...
    edgesSVG +=
      '<rect x="'+(lmx-18)+'" y="'+(lmy-12)+'" width="36" height="20" rx="10" fill="rgba(74,222,128,0.18)" stroke="#4ade80" stroke-width="1.2"/>'+
      '<text x="'+lmx+'" y="'+(lmy+2)+'" text-anchor="middle" fill="#4ade80" font-size="10" font-weight="700" '+ff+'>YES</text>'+
      '<rect x="'+(rmx-15)+'" y="'+(rmy-12)+'" width="30" height="20" rx="10" fill="rgba(248,113,113,0.18)" stroke="#f87171" stroke-width="1.2"/>'+
      '<text x="'+rmx+'" y="'+(rmy+2)+'" text-anchor="middle" fill="#f87171" font-size="10" font-weight="700" '+ff+'>NO</text>';
    drawEdges(node.left);
    drawEdges(node.right);
  }
  ...
  function drawNodes(node) {
    if (node.isLeaf) {
      var isYes = node.label === 'yes';
      var bg  = isYes ? 'rgba(22,101,52,0.55)'  : 'rgba(127,29,29,0.55)';
      var bdr = isYes ? '#4ade80' : '#f87171';
      var col = isYes ? '#4ade80' : '#f87171';
      var sym = isYes ? '\u2713 YES' : '\u2717 NO';
      nodesSVG +=
        ...
        '<text x="'+node._cx+'" y="'+(node._y+26)+'" text-anchor="middle" fill="'+col+
          '" font-size="18" font-weight="900" '+ff+'>'+sym+'</text>'+
        '<text x="'+node._cx+'" y="'+(node._y+44)+'" text-anchor="middle" fill="#aaaacc"'+
          ' font-size="10" '+ff+'>'+node.count.yes+'\u2713\u2002'+node.count.no+'\u2717</text>';
    } else {
      ...
      nodesSVG +=
        ...
        condSVG+
        '<text x="'+node._cx+'" y="'+(node._y+68)+'" text-anchor="middle" fill="#555577"'+
          ' font-size="9" '+ff+'>'+node.count.yes+'\u2713\u2002'+node.count.no+
          '\u2717\u2002(of '+node.items.length+' items)</text>';
      drawNodes(node.left);
      drawNodes(node.right);
    }
  }
...
function dtRunTest() {
  ...
  var results = TEST_ITEMS.map(function(item) {
    ...
    var isYes = path[path.length - 1].label === 'yes';
    return { item: item, isYes: isYes, path: path };
  });
  ...
  var FLY_MS     = 340; // ms to fly between nodes
  var PAUSE_ROOT = 700; // ms at root before first move
  var PAUSE_MID  = 550; // ms at internal nodes mid-path
  var PAUSE_LEAF = 280; // ms at leaf before landing
  var LW = 178;
  var ICON_S = 22, ICON_G = 3;
  var maxPerRow = Math.max(1, Math.floor((LW - 16) / (ICON_S + ICON_G)));
  ...
  function landInLeaf(leafNode, item, isYes) {
    var svgEl = document.getElementById('dt-test-tree-svg');
    ...
    if (dt.labelFeature && dt.labelValue !== '') {
      var gt  = String(_fval(item, dt.labelFeature)) === String(dt.labelValue);
      var ok  = (isYes === gt);
  ...
  function runNext(i) {
    ...
    setTimeout(step, 260); // brief pause after pop-in before first highlight
  }
