...
function _renderTree() {
  ...
  var isExploreMode = (typeof exploreMode !== 'undefined' && exploreMode);
  if (legendEl) {
    if (isExploreMode) {
      legendEl.innerHTML =
        '<span class="dt-leg-split">\u25c7 Decision Split \u2014 TRUE goes left, FALSE goes right</span>' +
        '<span class="dt-leg-yes" style="color:#4ade80">\u25a0 Correctly classified</span>' +
        '<span class="dt-leg-no" style="color:#f87171">\u25a1 Incorrectly classified (pulsing)</span>';
  ...
  document.getElementById('dt-tree-note').innerHTML = isExploreMode
    ? 'Showing <strong>all ' + (typeof ITEMS !== 'undefined' ? ITEMS.length : count) + ' dataset items</strong> — find the ones the broken model gets wrong.' +
      '<span class="dt-note-tips"><span>Hover items for details</span><span>Green border = correctly classified</span><span>Red pulsing = misclassified</span></span>'
  ...
  // In explore mode: classify ALL ITEMS into their leaf nodes for display
  var _exploreItemsByLeaf = null;
  if (isExploreMode && typeof ITEMS !== 'undefined') {
    _exploreItemsByLeaf = {};
    var _leafCounter = 0;
    (function _tagLeaves(node) {
      if (node.isLeaf) { node._leafTag = _leafCounter++; }
      else { _tagLeaves(node.left); _tagLeaves(node.right); }
    })(dt.tree);
    ITEMS.forEach(function(item) {
      var leaf = (function _findLeaf(node) {
        if (node.isLeaf) return node;
        var val = _fval(item, node.feature);
        var goLeft = node.isNumeric ? (Number(val) <= node.splitVal) : (String(val) === String(node.splitVal));
        return _findLeaf(goLeft ? node.left : node.right);
      })(dt.tree);
      if (!_exploreItemsByLeaf[leaf._leafTag]) _exploreItemsByLeaf[leaf._leafTag] = [];
      _exploreItemsByLeaf[leaf._leafTag].push(item);
    });
  }
  ...
  function drawNodes(node) {
    ...
    if (node.isLeaf) {
      ...
      var _leafDisplayItems = (_exploreItemsByLeaf !== null && node._leafTag !== undefined)
        ? (_exploreItemsByLeaf[node._leafTag] || [])
        : node.items;
      var maxPerRow = Math.max(1, Math.floor((LW - 16) / (ICON_S + ICON_G)));
      var iconRows  = _leafDisplayItems.length ? Math.ceil(_leafDisplayItems.length / maxPerRow) : 0;
      ...
      _leafDisplayItems.forEach(function(item, idx) {
        var row      = Math.floor(idx / maxPerRow);
        var colIdx   = idx % maxPerRow;
        var rowStart = row * maxPerRow;
        var rowCount = Math.min(maxPerRow, _leafDisplayItems.length - rowStart);
        ...
        // Explore mode: color border based on correct/incorrect classification
        var iStroke = 'rgba(255,255,255,0.12)', iStrokeW = 1, iAnim = '';
        if (_exploreItemsByLeaf !== null) {
          var iKey = String(item.id);
          var gt = (typeof exploreGroundTruth !== 'undefined') ? exploreGroundTruth[iKey] : undefined;
          if (gt !== undefined) {
            var predicted = _classify(dt.tree, item);
            var isCorrect = (dt.labelFeature && dt.labelValue === '')
              ? String(predicted) === String(gt)
              : (predicted === true || predicted === 'yes') === Boolean(gt);
            if (isCorrect) {
              iStroke = '#4ade80'; iStrokeW = 2;
            } else {
              iStroke = '#f87171'; iStrokeW = 2;
              iAnim = '<animate attributeName="stroke-opacity" values="0.25;1;0.25" dur="1.8s" repeatCount="indefinite"/>';
            }
          }
        }
        ...
        nodesSVG +=
          '<g data-item-id="' + _escSvg(String(item.id)) + '" style="cursor:pointer">' +
            '<rect x="' + (ix - 1) + '" y="' + (iy - 1) + '" width="' + (ICON_S + 2) + '" height="' + (ICON_S + 2) + '" rx="3" ' +
              'fill="rgba(255,255,255,0.05)" stroke="' + iStroke + '" stroke-width="' + iStrokeW + '">' +
              iAnim +
            '</rect>' +
  ...
  if (accBanner) {
    if (isExploreMode && typeof exploreGroundTruth !== 'undefined') {
      // Explore mode: accuracy against all ground-truth items
      var gtKeys = Object.keys(exploreGroundTruth);
      var expCorrect = 0;
      gtKeys.forEach(function(id) {
        var item = ITEMS.find(function(i) { return String(i.id) === id; });
        if (!item) return;
        var predicted = _classify(dt.tree, item);
        var gt = exploreGroundTruth[id];
        var isCorrect = (dt.labelFeature && dt.labelValue === '')
          ? String(predicted) === String(gt)
          : (predicted === true || predicted === 'yes') === Boolean(gt);
        if (isCorrect) expCorrect++;
      });
      var expTotal = gtKeys.length;
      if (expTotal > 0) {
        var pct   = Math.round(expCorrect / expTotal * 100);
        var color = _accColor(pct);
        var msg   = _accMsg(pct);
        accBanner.innerHTML =
          '<div class="dt-train-acc-row">' +
            '<span class="dt-train-acc-label">Model Accuracy on Evaluation Set</span>' +
            '<span class="dt-train-acc-score" style="color:' + color + '">' +
              expCorrect + ' / ' + expTotal + ' correct' +
              '<span>(' + pct + '%)</span>' +
            '</span>' +
          '</div>' +
    ...
    } else if (!dt.labelFeature) {
      // Binary question mode: no ground truth, hide banner
      accBanner.classList.add('hidden');
    } else {
      // Normal feature mode: show training accuracy
