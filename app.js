...
function buildCards() {
  ...
  grid.innerHTML = _visibleItems().map(item => {
    ...
    const isMisclassified = exploreMode && (function() { var c = getItemClassification(item); return c && !c.correct; })();

    return `
<div class="item-card${isMisclassified ? ' card--misclassified' : ''}" data-item-id="${item.id}" style="--rarity-color:${borderColor}; --glow-color:${glowColor}" onclick="toggleCard(event,this)">
  ${clfBadge}
...
function loadDataset(id) {
  ...
  if (!ds.exploreFile) exploreMode = false;

  const files = [ds.testFile, ds.dataFile];
  if (ds.exploreFile) files.push(ds.exploreFile); // always load so stories are available in both modes
...
function launchApp() {
  ...
  if (exploreMode && window._pendingExploreModel) {
    ...
    // Extract stories from the explore file even in build mode
    _userStories = (window._pendingExploreModel && Array.isArray(window._pendingExploreModel.userStories))
      ? window._pendingExploreModel.userStories
      : [];
    window._pendingExploreModel = null;
...
function _activateExploreMode(data) {
  ...
  // Open the tree view immediately so students see the pre-built broken model
  if (dt.tree) {
...
function _buildExploreTree() {
  ...
  var trainingData = labeledItems.map(function(item) {
    ...
    // Multi-class: labels are class strings. Binary: labels are booleans → 'yes'/'no'.
...
function getItemClassification(item) {
  ...
  var predicted = _classify(dt.tree, item); // leaf label string ('yes'/'no' or class)
  // Multi-class: compare string labels directly.
  if (dt.labelFeature && dt.labelValue === '') {
    var groundTruth = String(exploreGroundTruth[key]);
    return { correct: String(predicted) === groundTruth, predicted: predicted };
  }
  // Binary: compare booleans.
  var groundTruth = Boolean(exploreGroundTruth[key]);
  var predictedBool = (predicted === true || predicted === 'yes');
  return { correct: predictedBool === groundTruth, predicted: predicted };
}
